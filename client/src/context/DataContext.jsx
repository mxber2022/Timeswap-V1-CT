...
const DataContextProvider = ({ children }) => {
  ...
  const borrow = async (maturity, assetTo, dueTo, xDecrease, yIncrease, zIncrease, data) => {
    ...
    try {
      ...
      const tx = await pairContract.borrow(maturity, assetTo, dueTo, xDecrease, yIncrease, zIncrease, data)
      await tx.wait()
    } catch (error) {
  ...
  const burn = async (maturity, assetTo, collateralTo, liquidityIn) => {
    ...
    try {
      const pairContract = await getContractInstance(pairAddress, pairAbi)
      const tx = await pairContract.burn(maturity, assetTo, collateralTo, liquidityIn)
      await tx.wait()
    } catch (error) {
  ...
  const lend = async (maturity, bondTo, insuranceTo, xIncrease, yDecrease, zDecrease, data) => {
    ...
    try {
      const pairContract = await getContractInstance(pairAddress, pairAbi)
      const tx = await pairContract.lend(maturity, bondTo, insuranceTo, xIncrease, yDecrease, zDecrease, data)
      await tx.wait()
    } catch (error) {
  ...
  const mint = async (maturity, liquidityTo, dueTo, xIncrease, yIncrease, zIncrease, data) => {
    ...
    try {
      const pairContract = await getContractInstance(pairAddress, pairAbi)
      const tx = await pairContract.mint(maturity, liquidityTo, dueTo, xIncrease, yIncrease, zIncrease, data)
      await tx.wait()
    } catch (error) {
